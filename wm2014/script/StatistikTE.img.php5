...
for ($SpielCounter=$AnzSpiele;$SpielCounter>=1;$SpielCounter--)
{
  ...
  foreach ($Tabelle as $TippEintrag)
  {
    ...
    $MaxShift = 0.15;
    $LineData[$PlotIndex][$SpielCounter-1] = $AnzSpieler-($TippEintrag->iPlatz)-$PlotIndex/$AnzSpieler*$MaxShift;
  }
...
$graph->SetScale("int",0,$AnzSpieler-1,0,$MaxSpiele-1);
...
if ($FilterGesetzt)
  $graph->title->Set("Tabellenentwicklung. Filter: Platz $FilterVon bis $FilterBis");
else
  $graph->title->Set("Tabellenentwicklung");
$graph->xaxis->title->Set("Spiele");
//$graph->xaxis->SetFont(FF_TREBUCHE,FS_NORMAL,7);
$graph->xaxis->SetPos(-0.5);
$graph ->xgrid->Show(true);

$graph->yaxis->title->Set("Platz");
//$graph->yaxis->SetFont(FF_TREBUCHE,FS_NORMAL,7);

$graph->SetTickDensity(TICKD_DENSE,TICKD_DENSE);

//$graph->title->SetFont(FF_TREBUCHE,FS_BOLD, 14);
$graph->title->SetMargin(15,0,0,0);
//$graph->yaxis->title->SetFont(FF_TREBUCHE,FS_NORMAL,12);
//$graph->xaxis->title->SetFont(FF_TREBUCHE,FS_NORMAL,12);

//$graph->legend->SetFont(FF_TREBUCHE,FS_NORMAL,7);
