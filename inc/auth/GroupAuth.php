...
//public:
	...
	function GroupAuth($options = null) {
		$this->_options['groupsTable'] = GROUP_TABELLE;
		$this->_options['groupIdField'] = 'GruppenID';
		$this->_options['groupNameField'] = 'Name';
		$this->_options['usersGroupsTable'] = USERGROUP_TABELLE;
	 ...
	 * Force the user to identify himself (If he hasn't done it before) and
...
//protected:
	...
	function _getGroups($userId) {
		...
		if ($rs === false) {
			trigger_error('SQL Syntax error when reading the user\'s groups. (SQL: '.$sql.')',
				E_USER_ERROR);
			die();
		} else {
